...
public class BigBoid : MonoBehaviour
{
    ...
    {
        pathOb = GameObject.Find("SpawnLocation");
        path = pathOb.GetComponent<Path>();
    ...
    {
        pathOb = GameObject.Find("SpawnLocation");
        target = pathOb.GetComponent<LightsSpawn>().target;
        ...
        //When car arrives at light, choose different light to target
        if(this.transform.position.x < target.x + 1 && this.transform.position.z < target.z + 1)
        {
            pathOb.GetComponent<LightsSpawn>().random = (int)Random.Range(0, 10);
        }
